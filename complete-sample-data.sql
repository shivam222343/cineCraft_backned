-- CineCraft Media - Complete Sample Data Script
...
-- =============================================
-- ADD BOOKINGS (using existing service IDs from database)
-- =============================================
-- Get the first few service IDs that exist
WITH existing_services AS (
  SELECT id FROM services ORDER BY id LIMIT 3
)
INSERT INTO bookings (user_id, service_id, name, email, phone, service, date, time, message, image, attachments, status, estimated_price, notes, created_at, updated_at) 
SELECT 
  NULL,
  (SELECT id FROM existing_services OFFSET 0 LIMIT 1),
  'Michael Rodriguez', 
  'michael@weddingplanning.com', 
  '+1 (555) 789-0123', 
  'Wedding Photography Premium', 
  '2024-04-15', 
  '09:00', 
  'Planning a destination wedding in Tuscany. Need comprehensive photography coverage including pre-wedding events, ceremony, and reception. Looking for cinematic style with natural lighting.', 
  'https://res.cloudinary.com/demo/image/upload/sample.jpg', 
  ARRAY['venue-photos.pdf', 'timeline.docx', 'inspiration-board.jpg'], 
  'confirmed', 
  '$3,500 - $4,500', 
  'Client confirmed premium package with engagement session', 
  '2024-01-18 14:30:00', 
  NOW();
...
SELECT 
  NULL,
  (SELECT id FROM existing_services OFFSET 1 LIMIT 1),
  'Jennifer Park', 
  'jennifer@techcorp.com', 
  '+1 (555) 456-7891', 
  'Corporate Training Videos', 
  '2024-03-20', 
  '10:00', 
  'Need to create a series of training videos for our new employee onboarding program. Content should be engaging and professional with interactive elements for better learning outcomes.', 
  NULL, 
  ARRAY['training-outline.pdf', 'brand-guidelines.pdf'], 
  'pending', 
  '$2,800 - $3,500', 
  'Waiting for script approval from client', 
  '2024-01-20 11:15:00', 
  NOW();
...
SELECT 
  NULL,
  (SELECT id FROM existing_services OFFSET 2 LIMIT 1),
  'Robert Kim', 
  'robert@luxuryrealty.com', 
  '+1 (555) 234-5678', 
  'Real Estate Virtual Tours', 
  '2024-03-05', 
  '13:00', 
  'High-end property in Beverly Hills needs virtual tour with drone footage. Property is 8,000 sq ft with pool, gardens, and city views. Need premium presentation for international buyers.', 
  'https://res.cloudinary.com/demo/image/upload/house.jpg', 
  ARRAY['property-details.pdf', 'floor-plans.pdf'], 
  'in-progress', 
  '$2,200 - $2,500', 
  'Drone permits obtained, shooting scheduled for next week', 
  '2024-01-22 09:45:00', 
  NOW();
...
-- =============================================
-- ADD PORTFOLIO EVENTS DATA (if tables exist)
-- =============================================
DO $$
BEGIN
    -- Check if portfolio_events table exists and add data
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'portfolio_events') THEN
...
SELECT 'Sample data added successfully! Admin user created with email: shivamdombe1@gmail.com and password: Admin@123' as status;
